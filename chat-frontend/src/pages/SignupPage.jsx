  ...
  async function onSubmit(values) {
    try {
      ...
      if (data.token && data.user) {
        ...
        toast.success("Welcome to Teamat!");
        navigate("/app");
      } else {
      ...
      <div className="relative w-full max-w-sm animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-6">
          ...
          <div className="space-y-2">
            <label className="text-[11px] text-[var(--text-muted)] font-black uppercase tracking-widest ml-1">User Name</label>
          ...
          {/* PASSWORD */}
          <div className="space-y-2">
            <label className="text-[11px] text-[var(--text-muted)] font-black uppercase tracking-widest ml-1">Password</label>
            <div className="relative group">
              <div className="absolute inset-y-0 left-4 flex items-center text-[var(--text-secondary)] group-focus-within:text-[var(--accent-primary)] transition-colors">
        ...
        <div className="mt-8 text-center pt-6 border-t border-[var(--border-primary)]/50">
          <p className="text-sm text-[var(--text-muted)] font-bold">
            ...
            <Link to="/login" className="text-[var(--accent-primary)] hover:underline font-black ml-1 transition">
              Log in
            </Link>
          </p>
        </div>
